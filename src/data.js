export const category = [
  {
    _id: 'Electronic',
    img: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTmgH3Hsh3lUnknOZ54DIupopR5Hk3WesgruA&usqp=CAU',
    name: 'Electronics item',
  },
  {
    _id: 'Chemical',
    img: 'https://i.imgur.com/gQVQC06.jpg',
    name: 'Chemical Equipment',
  },
  {
    _id: 'Books',
    img: 'https://images.theconversation.com/files/45159/original/rptgtpxd-1396254731.jpg?ixlib=rb-1.1.0&q=45&auto=format&w=754&fit=clip',
    name: 'Books',
  },
]

export const slideData = [
  {
    id: 1,
    img: '/image/bookHome.png',

    title: 'View Products',
    desc: 'Why every time buy new product at higher price when you get it at lesser price.',
    bg: 'edf2fb',
    link: '/products',
    name: 'View',
  },
  // {
  //   id: 2,
  //   img: '/image/Addfile.png',
  //   title: 'Add Products',
  //   desc: 'If you have any product which you dont need add here. So Other Students can Buy it.',
  //   bg: '4D77FF',
  //   link: '/addItem',
  //   name: 'Add Products',
  // },
  // {
  //   id: 3,
  //   img: '/image/home1.png',
  //   title: 'Recent Items',
  //   desc: 'View Recently Added item And buy it . ',
  //   bg: 'ffffff',
  //   link: '/',
  //   name: 'Recent Items',
  // },

  // {
  //   id: 4,
  //   img: '/image/request.png',
  //   title: 'Request Item',
  //   desc: 'If you dont find what you looking for Request the product.Click below to Request.',
  //   bg: 'ffffff',
  //   link: '/requestItem',
  //   name: 'Request Product',
  // },
]
